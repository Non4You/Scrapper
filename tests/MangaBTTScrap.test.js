...
describe("MangaBTT Class", () => {
    ...
    beforeEach(async () => {
        asuraScrap = createSiteScrapClass("MangaBTT", 3);
    ...
    test("test access main MangaBTT page", async () => {
        expect(asuraScrap.mainUrl).not.toBe("");
        await expect([200, 304]).toContain(await basicActionBrowser.accessPage(asuraScrap.mainUrl));
    });

    test("test MangaBTT site Icon", async () => {
    ...
    test("test MangaBTT check get manga links", async () => {
        await basicActionBrowser.accessPage(asuraScrap.mainUrl)
    ...
    test("test MangaBTT check manga info", async () => {
        ...
        expect(parseFloat(asuraScrap.extractNumber(mangaInfoSources[0][2]))).toBeGreaterThanOrEqual(0);
        expect(parseFloat(asuraScrap.extractNumber(mangaInfoSources[0][2]))).toBeLessThanOrEqual(10);
    ...
    test("test MangaBTT check access manga chapter", async () => {
        ...
        await expect([200, 304]).toContain(res);
    });

    test("test MangaBTT check manga chapter images", async () => {
        await basicActionBrowser.accessPage(asuraScrap.mainUrl);
        var allMangaSources = await headLessBrowser.getDataEvaluateLoop(asuraScrap.mangaInfo[0], asuraScrap.mangaInfo[1]);
        await basicActionBrowser.accessPage(allMangaSources[0][1]);
        if (asuraScrap.mangaChaptersGatheringMethod === 1) {
