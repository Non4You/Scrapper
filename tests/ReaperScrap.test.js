...
describe("ReaperScan Class", () => {
    let asuraScrap;
    let basicActionBrowser;
    let headLessBrowser;

    beforeEach(async () => {
        asuraScrap = createSiteScrapClass("ReaperScan", 1);
    ...
    test("test Reaper check get manga links", async () => {
        await basicActionBrowser.accessPage(asuraScrap.mainUrl)
    ...
    test("test Reaper check pagination", async () => {
        await basicActionBrowser.accessPage(asuraScrap.mainUrl)
        ...
        var res = await headLessBrowser.evaluateAndclickOnSelectorIf(asuraScrap.pagination[0], asuraScrap.pagination[1], asuraScrap.pagination[2], "Next Page Error :");
    ...
    test("test Reaper check manga", async () => {
        await basicActionBrowser.accessPage(asuraScrap.mainUrl);
        var allMangaSources = await headLessBrowser.getDataEvaluateLoop(asuraScrap.mangaInfo[0], asuraScrap.mangaInfo[1]);
    ...
    test("test Reaper check manga info", async () => {
    ...
    test("test Reaper check access manga chapter", async () => {
        await basicActionBrowser.accessPage(asuraScrap.mainUrl);
        var allMangaSources = await headLessBrowser.getDataEvaluateLoop(asuraScrap.mangaInfo[0], asuraScrap.mangaInfo[1]);
    ...
    test("test Reaper check manga chapter images", async () => {
        await basicActionBrowser.accessPage(asuraScrap.mainUrl);
        var allMangaSources = await headLessBrowser.getDataEvaluateLoop(asuraScrap.mangaInfo[0], asuraScrap.mangaInfo[1]);
        await basicActionBrowser.accessPage(allMangaSources[0][1]);
        if (asuraScrap.mangaChaptersGatheringMethod === 1) {
